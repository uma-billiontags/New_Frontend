...
import { Table, Button, Input, InputNumber, Modal, Switch, Form, message } from "antd";
...
export default function Payment_Terms() {
    ...
    const openEdit = (record: PaymentTerm) => { setEditing(record); form.setFieldsValue(record); setModalOpen(true); };
    ...
    const handleSave = async () => {
        ...
        const url = isEdit
            ? `${BASE_URL}/categories/edit_payment_term/${editing.id}/`
            : `${BASE_URL}/categories/create_payment_term/`;
        try {
            ...
            if (res.ok) {
                message.success(isEdit ? "Payment term updated" : "Payment term added");
                setModalOpen(false);
                fetchData();
            } else {
    ...
    const handleDelete = (record: PaymentTerm) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const res = await fetch(`${BASE_URL}/categories/delete_payment_term/${record.id}/`, {
                        method: "DELETE",
                        headers: { "ngrok-skip-browser-warning": "1" },
                    });
    ...
    return (
        <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, borderBottom: "1px solid var(--border)", paddingBottom: 8 }}>
                <div>
                    <h1 style={{ fontSize: 16, fontWeight: 600, margin: 0, color: "var(--text-primary)" }}>Payment Terms</h1>
                    <p style={{ fontSize: 9, color: "var(--text-muted)", margin: "4px 0 0", fontWeight: 500, letterSpacing: "0.04em", textTransform: "uppercase" }}>
                        Define when invoice payments are due
                    </p>
                </div>
            ...
            >
                <Form form={form} layout="vertical">
                    <div className="db-form-row">
                        ...
                        <Form.Item label="Days" name="days" rules={[{ required: true, message: "Required" }]}>
