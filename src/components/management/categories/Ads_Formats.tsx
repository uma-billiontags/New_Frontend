...
export default function Ads_Formats() {
    const [data, setData] = useState<AdsFormat[]>([]);
    const [loading, setLoading] = useState(true);
    const [search, setSearch] = useState("");
    const [modalOpen, setModalOpen] = useState(false);
    const [editing, setEditing] = useState<AdsFormat | null>(null);
    const [saving, setSaving] = useState(false);
    const [form] = Form.useForm();

    const fetchData = useCallback(() => {
        setLoading(true);
        fetch(`${BASE_URL}/categories/get_all_ads_formats/`, {
            headers: { "ngrok-skip-browser-warning": "1" },
        })
            .then((r) => r.json())
            .then((d) => setData(Array.isArray(d) ? d : d.results || []))
            .catch(() => message.error("Failed to load ad formats"))
    ...
    const openEdit = (record: AdsFormat) => { setEditing(record); form.setFieldsValue(record); setModalOpen(true); };
    ...
    const handleSave = async () => {
        ...
        const url = isEdit
            ? `${BASE_URL}/categories/edit_ads_format/${editing.id}/`
            : `${BASE_URL}/categories/create_ads_format/`;
        try {
            ...
            if (res.ok) {
                message.success(isEdit ? "Ad format updated" : "Ad format added");
                setModalOpen(false);
                fetchData();
            } else {
    ...
    const handleDelete = (record: AdsFormat) => {
        Modal.confirm({
            title: "Delete ad format?",
            ...
            onOk: async () => {
                try {
                    const res = await fetch(`${BASE_URL}/categories/delete_ads_format/${record.id}/`, {
                        method: "DELETE",
                        headers: { "ngrok-skip-browser-warning": "1" },
                    });
    ...
    const columns: ColumnsType<AdsFormat> = [
    ...
    return (
        <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, borderBottom: "1px solid var(--border)", paddingBottom: 8 }}>
                <div>
                    <h1 style={{ fontSize: 16, fontWeight: 600, margin: 0, color: "var(--text-primary)" }}>Ad Formats</h1>
                    <p style={{ fontSize: 9, color: "var(--text-muted)", margin: "4px 0 0", fontWeight: 500, letterSpacing: "0.04em", textTransform: "uppercase" }}>
                        Manage ad format types used in insertion orders
                    </p>
                </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                ...
                <span style={{ fontSize: 12, color: "var(--text-muted)", marginLeft: "auto" }}>
                    {filtered.length} of {data.length} formats
                </span>
            </div>
            ...
            <div style={{ background: "var(--bg-card)", borderRadius: 14, border: "1px solid var(--border)", overflow: "hidden", boxShadow: "var(--shadow-card)" }}>
                <Table
                    ...
                    pagination={{
                        ...
                        showTotal: (total, range) => `${range[0]}–${range[1]} of ${total} formats`,
            ...
            <Modal
                ...
                title={editing ? "Edit Ad Format" : "Add Ad Format"}
            ...
            >
                <Form form={form} layout="vertical">
                    <Form.Item label="Title" name="title" rules={[{ required: true, message: "Required" }]}>
                        <Input placeholder="e.g. Banner, Video, Native" />
                    </Form.Item>
