...
// variant="sender"   -> email badge renders in BLUE
// variant="receiver" -> email badge renders in GREEN
function SenderCell({ raw, variant = "sender" }: { raw: string; variant?: "sender" | "receiver" }) {
    const { name, email } = parseSender(raw);
    const isNameDifferentFromEmail = !!email && email !== name;
    // Show the badge whenever we actually have an email — even when name === email
    // (i.e. a plain "finance@billiontags.com" address with no "Display Name <...>" wrapper).
    // Previously this required email !== name, which silently hid the badge for plain addresses.
        ...
        <div style={{ lineHeight: 1.4 }}>
            ...
            {showBadge ? (
                <div style={{
                    ...
                    border: `1px solid ${color}`, marginTop: isNameDifferentFromEmail ? 2 : 0,
...
// ── Attachment List (shared by table cell + modal) ───────────────────────────
...
// ── Stat Card — clickable, matches All_Campaigns.tsx exactly ─────────────────
...
// ── Toast — matches All_Campaigns.tsx ─────────────────────────────────────────
        ...
        >
            ...
            <div style={{
                ...
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                    Added {fmtDateTime(lead.created_at)}
                </span>
...
export default function Leads() {
    ...
    const handleGenerateClient = (lead: Lead) => {
        const { email } = parseSender(lead.sender);
        // opens onboarding in a new tab, prefilled with the lead's sender email
    ...
    useEffect(() => {
        ...
        channel.onmessage = (event) => {
            if (event.data?.type === "client_created") {
                setLeads((prev) =>
                    prev.map((l) =>
                        l.id === event.data.leadId ? { ...l, client_id: event.data.client_id } : l
                    )
                );
                showToast(`Client ${event.data.client_id} created and linked.`);
            }
        };
    ...
    const columns: ColumnsType<Lead> = [
            ...
            render: (_: any, record: Lead) =>
                record.client_id ? (
                    <span style={{
                        ...
                        padding: "2px 10px", borderRadius: 6, display: "inline-block", whiteSpace: "nowrap",
                    }}>{record.client_id}</span>
                ) : (
                    <Button size="small" onClick={() => handleGenerateClient(record)}
                        style={{ fontSize: 11, fontWeight: 600, height: 24, borderRadius: 6 }}>
                        + Generate
                    </Button>
                ),
        },
        ...
        },
        {
            ...
            key: "receiver",
            width: 200,
            render: (v: string) => <SenderCell raw={v} variant="receiver" />,
        },
        {
            title: "Attachments",
            key: "attachments",
    ...
    return (
        <div>
            ...
            <div style={{
                ...
                <div>
                    ...
                    <p style={{
                        ...
                        Incoming email leads
                    </p>
                </div>
            </div>

            {/* ── Stat Cards — clickable filters, matches All_Campaigns.tsx ── */}
            ...
            <div style={{
                ...
                border: "1px solid var(--border)", marginBottom: 16, marginTop: 4,
                display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap",
            }}>
                <Input
                    placeholder="Search by sender, receiver, subject, thread ID…"
            ...
            <style>{`
                .lead-detail-modal .ant-modal-content {
                    padding: 0 !important;
