...
import {
    ...
    InboxOutlined, FileImageOutlined, UploadOutlined, PictureOutlined,
...
function readImageMeta(file: File): Promise<{ dimensions: string; aspect_ratio: string; previewUrl: string }> {
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            resolve({
                ...
                aspect_ratio: aspectRatioOf(img.naturalWidth, img.naturalHeight),
                previewUrl: url,
            });
        };
        img.onerror = () => resolve({ dimensions: "", aspect_ratio: "", previewUrl: url });
        img.src = url;
    });
}
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function Image_Creatives_Campaign() {
    ...
    const perRowFileInputRef = useRef<HTMLInputElement>(null);
    ...
    // ── Attach a file to a specific row (used by per-row "choose file") ──
    ...
    // ── Bulk add: each dropped/browsed file becomes its own row ──
    const addFilesAsRows = async (files: FileList | File[]) => {
        const imageFiles = Array.from(files).filter((f) => f.type.startsWith("image/"));
        const skipped = files.length - imageFiles.length;
        if (skipped > 0) message.warning(`Skipped ${skipped} non-image file(s).`);
    ...
    const openPerRowBrowse = (rowId: string) => {
        pendingRowIdRef.current = rowId;
        perRowFileInputRef.current?.click();
    };
            ...
            <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
                        ...
                        <span style={{ fontWeight: 600, color: "var(--text-primary)" }}>Image Bulk Upload</span>
                    </div>
                </div>
            ...
            <div style={{ padding: "24px 32px", maxWidth: 1300, margin: "0 auto" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
                    <div style={{
                        ...
                        <FileImageOutlined style={{ fontSize: 20, color: "var(--blue)" }} />
                    </div>
                    <div>
                            ...
                            {rows.length === 0 && (
                                <tr>
                                    <td colSpan={11} style={{ padding: "28px", textAlign: "center", color: "var(--text-muted)" }}>
                                        No creatives added yet — use “Add Image Creative,” “Third Party,” or drop files below.
                                    </td>
                                </tr>
                            )}
                            {rows.map((row, idx) => {
                                ...
                                return (
                                    <tr key={row.rowId} style={{ borderBottom: "1px solid var(--border)" }}>
                                        ...
                                        <Cell width={190}>
                                            {isTP ? (
                                                ...
                                                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                                    {row.previewUrl ? (
                                                        <img src={row.previewUrl} alt="" style={{ width: 30, height: 30, objectFit: "cover", borderRadius: 4, border: "1px solid var(--border)" }} />
                                                    ) : (
                                                        <div style={{
                                                            width: 30, height: 30, borderRadius: 4, background: "var(--bg-page)",
                                                            border: "1px dashed var(--border-strong)", display: "flex",
                                                            alignItems: "center", justifyContent: "center", flexShrink: 0,
                                                        }}><FileImageOutlined style={{ fontSize: 12, color: "var(--text-muted)" }} /></div>
                                                    )}
                                        ...
                                        <Cell width={170}>
                                            {isTP ? <span style={{ color: "var(--text-muted)" }}>—</span> : (
                                                <Input size="small" placeholder="…trackimp…" value={row.appended_html_tag || ""}
                ...
                {/* Dropzone — drop multiple images to add rows in one go */}
                <div
                    onDrop={handleDrop}
                ...
                <input ref={perRowFileInputRef} type="file" style={{ display: "none" }} />
            </div>
        </div>
    );
}